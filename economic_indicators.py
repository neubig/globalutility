...
ETHNOLOGUE_COUNTRY_CODES = f"{current_dir}/economic_indicators_data/CountryCodes.tab"
ETHNOLOGUE_LANGUAGE_INDEX = f"{current_dir}/economic_indicators_data/LanguageIndex.tab"
LANGUAGE_MAPPINGS = f"{current_dir}/economic_indicators_data/language_mappings.txt"
DATA_DIR = f"{current_dir}/economic_indicators_data/wits_en_trade_summary_allcountries_allyears"
LANGUAGE_TO_COUNTRY_INDEX = (
    f"{current_dir}/economic_indicators_data/language_to_countries_map_small.txt"
)
LANGUAGE_TO_COUNTRY_INDEX_WEIGHTED = (
    f"{current_dir}/economic_indicators_data/language_to_countries_map_weighted.txt"
)
NOMINAL_GDP = f"{current_dir}/economic_indicators_data/NOMINAL_GDP.tsv"
...
def helper_for_years(lines, indicator_type, indicator, year=2017):
    ...
    if "absolute" in indicator:
        try:
            for l in lines[1:]:
                if (
                    l[1].strip() == "World"
                    and l[2].strip() == "All Products"
                    and l[3].strip() == indicator_type
                    and ("in US$ Mil" in l[4])
                ):
                    if l[ind].strip():
    ...
    for l in lines[1:]:
        ...
        if l[ind].strip():
            # print("came in 1")
            if (
                l[2].strip() == "All Products"
                and l[3] == indicator_type
                and l[1].strip() != "World"
            ):
                # print("came in 2")
...
def read_economic_indicators(languages=[], ind_type="Import", ind="Top 5 absolute"):
    ...
    for f in os.listdir(DATA_DIR):
        d, country, country_abbreviation = read_csv_file(
            os.path.join(DATA_DIR, f), indicator_type=ind_type, indicator=ind
        )
        if d == -1:
            print(f)
        else:
                    ...
                    if country1 in data:
                        for country2 in countries2:
                            if country2 in data[country1]:
                                ...
                                if (l1, l2) in newdata:
                                    newdata[(l1, l2)] += data[country1]["others"] / (
                                        (N - 5) * denom
                                    )
                                else:
                                    newdata[(l1, l2)] = data[country1]["others"] / (
...
def read_absolute_economic_indicators2(
    languages=[], ind_type="Import", ind="Top 5 absolute"
):
    # expect a set of languages
    N = len(languages)
    print(f"Reading economic indicators for {N} languages: {languages}")
...
def read_absolute_economic_indicators(
    languages=[], ind_type="Import", ind="Top 5 absolute"
):
    # expect a set of languages
    N = len(languages)
