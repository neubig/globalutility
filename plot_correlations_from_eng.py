...
for INTEREST in 'ell,fra,spa,eng,cmn,swa,swe,fin,deu,hin'.split(','):
	...
	papers = [paper_num[INTEREST,l]+internal_paper_num[l, INTEREST] for l in languages]
	ec_dem1 = [(demand1[l,INTEREST]+demand1[INTEREST,l])/1000.0 for l in languages]
	ec_dem2 = [(demand2[l,INTEREST]+demand2[INTEREST,l])/1000.0 for l in languages]
	...
	for i in range(len(populations)):
		...
		if papers[i] > 25 or populations[i] > 0.3*mmax or utility[i] == 1:
			if lang in 'slv,hrv'.split(','):
				ax[0].annotate(lang,(populations[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(0,-10))
			elif lang in 'ron'.split(','):
				ax[0].annotate(lang,(populations[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(11,-3))
			elif lang in 'hin'.split(','):
				ax[0].annotate(lang,(populations[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(11,-4))
			elif lang in 'deu'.split(','):
				ax[0].annotate(lang,(populations[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(13,-5))
			elif lang in INTEREST.split(','):
				ax[0].annotate(lang,(populations[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(-6,4))
			else:
	...
	for i in range(len(ec_dem1)):
		...
		if papers[i] > 18 or ec_dem1[i] > mmax:
			if lang in 'slv,hrv'.split(','):
				ax[1].annotate(lang,(ec_dem1[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(0,-10))
			elif lang in 'deu,jpn'.split(','):
				ax[1].annotate(lang,(ec_dem1[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(13,-5))
			elif lang in INTEREST.split(','):
				ax[1].annotate(lang,(ec_dem1[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(-4,4))
			elif lang in 'spa'.split(','):
				ax[1].annotate(lang,(ec_dem1[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(-4,4))
			elif lang in 'ron'.split(','):
				ax[1].annotate(lang,(ec_dem1[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(2,4))
			elif lang in 'nld'.split(','):
				ax[1].annotate(lang,(ec_dem1[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(-2,2))
			else:
				ax[1].annotate(lang,(ec_dem1[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(0,4))
	if rho2[0] < 0.5:
	...
	ax[2].scatter(ec_dem2, utility, marker='o', s=10, facecolors='none', edgecolors='blue')
	...
	for i in range(len(ec_dem2)):
		...
		if papers[i] > 25 or ec_dem2[i] > mmax:
			if lang in 'slv,hrv'.split(','):
				ax[2].annotate(lang,(ec_dem2[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(0,-10))
			elif lang in 'deu,jpn'.split(','):
				ax[2].annotate(lang,(ec_dem2[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(13,-5))
			elif lang in INTEREST.split(','):
				ax[2].annotate(lang,(ec_dem2[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(0,4))
			elif lang in 'spa'.split(','):
				ax[2].annotate(lang,(ec_dem2[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(-4,4))
			elif lang in 'fra'.split(','):
				ax[2].annotate(lang,(ec_dem2[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(0,4))
			elif lang in 'ron'.split(','):
				ax[2].annotate(lang,(ec_dem2[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(2,4))
			elif lang in 'nld'.split(','):
				ax[2].annotate(lang,(ec_dem2[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(-2,2))
			else:
				ax[2].annotate(lang,(ec_dem2[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(0,4))
	if rho3[0] < 0.5:
	...
	ax[3].scatter(papers, utility, marker='o', s=10, facecolors='none', edgecolors='blue')
	ax[3].set_xlabel("#Papers", fontsize=11)
	for i in range(len(accuracy)):
		lang = languages[i]
		if papers[i] > 17 or utility[i] > 0.9:
			if lang in 'slv,hrv'.split(','):
				ax[3].annotate(lang,(papers[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(0,-10))
			elif lang in 'deu'.split(','):
				ax[3].annotate(lang,(papers[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(2,-10))
			elif lang in INTEREST.split(','):
				ax[3].annotate(lang,(papers[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(4,2))
			elif lang != 'kin':
				ax[3].annotate(lang,(papers[i],utility[i]),textcoords="offset points",ha='center',fontweight='bold',xytext=(0,4))
	if rho4[0] < 0.5:
	...
	ax[3].legend(handles, labels, loc='best', fontsize=11, fancybox=True, framealpha=0.7,  handlelength=0, handletextpad=0)

	plt.tight_layout()
	plt.subplots_adjust(wspace=0.2)
	plt.show()

	fig.savefig(f"figs/from_lang/bleu_correlations_from_{INTEREST}_L1only.pdf", quality=100, format='pdf')
