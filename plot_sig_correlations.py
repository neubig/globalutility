...
all_accs = constants.read_sig()
all_populations = constants.read_sig_populations()
all_populations_L1 = constants.read_sig_populations(L1only=True)
...
M = max(accuracy)
# utility = [(all_populations[l]/NS) * (all_accs[l,INTEREST]/M) for l in languages]
...
ax[1].scatter(
    populations_L1, utility, marker="o", s=10, facecolors="none", edgecolors="blue"
)
...
fig.savefig(f"figs/sig_correlations.pdf", quality=100, format="pdf")
